...
class ControllerModuleDSocialLike extends Controller {
	...
	public function index($setting) {
		...
		if ((($setting['language_id'] == (int)$this->config->get('config_language_id')) || ($setting['language_id'] == -1)) && (($setting['store_id'] == (int)$this->config->get('config_store_id')) || ($setting['store_id'] == -1))) {
			...
			if((!empty($setting['social_likes']['stumbleupon']) && $setting['social_likes']['stumbleupon']['enabled'] && !empty($_SERVER['HTTP_X_FORWARDED_PROTO'])) || (!empty($setting['social_likes']['stumbleupon']) && $setting['social_likes']['stumbleupon']['enabled'] && !empty($_SERVER['HTTPS']))){
